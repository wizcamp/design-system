...
def update_tokens_md(tokens: List[Dict[str, any]]) -> None:
    """Update tokens.md with Base Colors table."""
    print(f"📝 Updating {TOKENS_MD_PATH}...")
    
    if not TOKENS_MD_PATH.exists():
        print(f"❌ Error: {TOKENS_MD_PATH} does not exist")
        sys.exit(1)
    
    content = TOKENS_MD_PATH.read_text()
    
    # Find the Base Colors section
    ...
    for token in sorted(tokens, key=lambda t: t['name']):
        # Format hex with opacity if present
        light_hex_display = token['light_hex']
        if token.get('light_opacity'):
            opacity_pct = int(token['light_opacity'] * 100)
            light_hex_display = f"{light_hex_display} @ {opacity_pct}%"
        
        dark_hex_display = token['dark_hex']
        if token.get('dark_opacity'):
            opacity_pct = int(token['dark_opacity'] * 100)
            dark_hex_display = f"{dark_hex_display} @ {opacity_pct}%"
    ...
    # Replace the table (from header to next ### or end of section)
    pattern = r"(### Base Colors\n\n)(.*?)(\n\n#{1,3}\s|\n\n---|\Z)"
    ...
    TOKENS_MD_PATH.write_text(updated_content)
    print(f"✅ Updated Base Colors table ({len(tokens)} tokens)")
